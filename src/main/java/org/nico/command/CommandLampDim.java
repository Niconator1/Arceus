...
public class CommandLampDim extends CommandGateway {
    ...
    public void handleCommand(String text, Gateway gateway) {
        ...
        Pattern pattern = Pattern.compile("dimmen \\b\\d+\\b");
        ...
        final String name;
        if (matcherName.find()) {
            name = matcherName.group().substring(6, matcherName.group().length() - 7);
        } else {
            name = "";
        }
        try {
